...
**Status:** Phases 1–6 implemented (queue eligibility, host checkout, retry / allow-successors / computed `queue`, restore `queued` on startup, skip auto-review while coding successors exist, wait-reason / retry / queue-another UI).
